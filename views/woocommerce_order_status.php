...
<div class="postbox bbmsl-status bbmsl-bg">
	...
	<div class="header">
		<img src="<?php echo BBMSL::getLogoURL(); ?>" class="bbmsl-logo" />
	</div>
	<?php if( !BBMSL::matchOrderingMode( $order_id ) ) { ?>
		<div class="body">
			<p><?php echo __( 'Unable to read the order details if current environment setting is not aligned with this order\'s. Please switch the testing mode if you need to view this order details', 'bbmsl-gateway' ); ?></p>
	...
	<?php } else if( isset( $order_info ) && is_array( $order_info ) && sizeof( $order_info ) > 0 ) { ?>
		<div class="body">
			<?php if( isset( $order_status ) ) { ?>
				...
				<?php if( $order_status == 'OPEN' ) { ?>
					...
					<?php } else { ?>
						<p><?php echo __( 'Checkout link is not available.', 'bbmsl-gateway' ); ?></p>
					<?php } ?>
				<?php } ?>
			<?php } ?>
		</div>
		<table class="table" cellspacing="0">
			...
			<?php } ?>
			<?php if( isset( $order_info['currency'] ) ) { ?>
				<?php if( isset( $order_info['amount'] ) ) { ?>
				<tr>
				...
				<?php } ?>
				<?php if( isset( $order_info['totalRefundedAmount'] ) ) { ?>
				<tr>
					<th><?php echo __( 'Total Refunded Amount', 'bbmsl-gateway' ); ?></th>
					<td><?php echo esc_html( sprintf( '%s %s-', $order_info['currency'], number_format( doubleval( $order_info['totalRefundedAmount'] ), 2, '.', ',' ) ) ); ?></td>
				</tr>
				<?php } ?>
				<?php if( isset( $order_info['netAmount'] ) ) { ?>
				<tr>
					<th><?php echo __( 'Net Amount', 'bbmsl-gateway' ); ?></th>
					<td><?php echo esc_html( sprintf( '%s %s-', $order_info['currency'], number_format( doubleval( $order_info['netAmount'] ), 2, '.', ',' ) ) ); ?></td>
				</tr>
				<?php } ?>
			<?php } ?>
...
<?php if( !boolval( Utility::checkBoolean( Option::get( Constants::PARAM_GATEWAY_REFUND, 0) ) ) || ( isset( $order_status ) && in_array( $order_status, array( 'OPEN', 'SUCCESS', 'REFUNDED' ), true ) ) ) { ?>
...
.refund-actions:after{content:'<?php echo __( 'This order has already been refunded before.', 'bbmsl-gateway' ); ?>';
<?php } ?>
<?php } ?>
</style>